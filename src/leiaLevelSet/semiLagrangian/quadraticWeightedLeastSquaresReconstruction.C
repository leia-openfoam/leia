...
namespace Foam
{
    defineTypeNameAndDebug(quadraticWeightedLeastSquaresReconstruction, 0);
    addToRunTimeSelectionTable
    (
        slReconstruction,
        quadraticWeightedLeastSquaresReconstruction,
        Mesh
    );
    // Backward-compatible selection alias for the historical short name
    // "quadraticWLSQ": existing cases/studies/decks select that token, and the
    // class was renamed to the spelled-out OpenFOAM-style name. Both resolve here.
...
Foam::quadraticWeightedLeastSquaresReconstruction::quadraticWeightedLeastSquaresReconstruction
(
    const fvMesh& mesh
)
:
...
void Foam::quadraticWeightedLeastSquaresReconstruction::basis
(
    const vector& d,
    const label ncoeff,
    scalar* b
...
void Foam::quadraticWeightedLeastSquaresReconstruction::setupBasis()
{
...
void Foam::quadraticWeightedLeastSquaresReconstruction::build()
{
    setupBasis();
    const label nd = activeDirs_.size();
    ...
    forAll(stencilC_, c)
    {
        ...
        for (label k = 0; k < ncoeff; ++k)
        {
            float* row = MW_[c].data() + k*nNbr;   // single-precision cache
            for (label i = 0; i < nNbr; ++i)
            {
                row[i] = static_cast<float>(Minv(k, i)*wloc[i]);
            }
        }
    }
    ...
    if (nFallback > 0)
    {
        Info<< "quadraticWeightedLeastSquares: " << nFallback
...
void Foam::quadraticWeightedLeastSquaresReconstruction::update(const volScalarField& psiOld)
{
    ...
    forAll(stencilPsi_, c)
    {
        ...
        const float* MW = MW_[c].cdata();          // single-precision cache
        scalar* cf = coeffs_[c].data();
        for (label k = 0; k < ncoeff; ++k)
        {
            const float* row = MW + k*nNbr;
            scalar acc = 0;                         // accumulate in double
            for (label i = 0; i < nNbr; ++i)
            {
                acc += row[i]*dsp[i];               // float*double -> double
            }
            cf[k] = acc;
        }
    }
...
Foam::scalar Foam::quadraticWeightedLeastSquaresReconstruction::evaluateRaw
(
    const label c,
    const point& x
) const
