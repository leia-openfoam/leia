...
% feeds this article AND the reveal decks (geometrically-redistanced-level-set.template.html,
% geometrically-redistanced-level-set-negative-results.template.html).
...
\begin{table}[htbp]
  ...
  \label{tab:static}
  \input{data/tables/static_redistance_orders.tex}
\end{table}
...
\begin{table}[htbp]
  ...
  \resizebox{\linewidth}{!}{\input{data/tables/redistanceStatic2D_orders.tex}}
\end{table}
...
% \input{data/tables/grl_convergence_orders.tex}
...
line; the companion deck (\texttt{geometrically-redistanced-level-set-negative-results}) documents each with
