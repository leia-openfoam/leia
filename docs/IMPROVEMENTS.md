...
| [combined-source-terms/improvement-sdpls-combined.md](combined-source-terms/improvement-sdpls-combined.md) | `sdplsCombined` source model; note corrections (Π₁ error statement, symbol collisions) | new `src/leiaLevelSet/sdplsSource/sdplsCombined.{H,C}` and its line in `src/leiaLevelSet/sdplsSource/Make/files` (libleiaSdplsSource); `docs/combined-source-terms/*`; new unit case |
