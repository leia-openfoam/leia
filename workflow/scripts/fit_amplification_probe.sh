...
#                    `reuse` keeps the source case's EXISTING constant/polyMesh untouched,
#                    which is the only way to probe a mesh that cannot be regenerated by one
#                    command -- a 2:1 refinement transition built by leiaRefineHexMesh.py, or
#                    any mesh already on disk. Running blockMesh there would destroy it.
...
#     [pivotTol]     optional quadraticPivotTol override. Pass 0 to DISABLE the demotion to a
#                    linear fit, which answers whether the pivot test MASKS amplification:
#                    a demoted cell is fitted linearly and its Lambda is lower, so a low
#                    Lambda on a mesh with many demoted cells is not yet evidence.
#
...
if [ "$MESHER" = "reuse" ]; then
    [ -d "$OUT/constant/polyMesh" ] || { echo "ABORT: reuse asked but the source has no constant/polyMesh"; exit 2; }
else
...
case "$MESHER" in
  ...
  reuse)
      echo "  reusing the source mesh, $(ls "constant/polyMesh" | tr '\n' ' ')"
      true ;;
  snappyHexMesh)
...
if [ -n "$PIVOT" ]; then
    echo "  quadraticPivotTol OVERRIDDEN to $PIVOT"
    foamDictionary -entry levelSet/semiLagrangian/quadraticPivotTol -set "$PIVOT" system/fvSolution >/dev/null
fi
