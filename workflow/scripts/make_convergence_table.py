...
def main(argv=None):
    ...
    note = ("\\multicolumn{7}{l}{\\footnotesize $^\\dagger$stable envelope only; "
    ...
    with open(ext_path, "w") as fh:
        fh.write("% Auto-generated by workflow/scripts/make_convergence_table.py -- do not edit.\n")
        fh.write("\\begin{tabular}{llc" + "c" * len(METRICS) + "}\n\\toprule\n")
        fh.write("Case & Mesh & CFL & "
                 + " & ".join(lbl for _c, _o, lbl in METRICS) + " \\\\\n\\midrule\n")
        for r in order_rows:
            mesh_cell = r["mesh"] + ("$^\\dagger$" if r.get("hLimit") else "")
            fh.write(" & ".join(
                [pretty.get(r["case"], r["case"]), mesh_cell, str(r["cfl"])]
