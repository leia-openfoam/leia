...
def main():
    ...
    # production pairs: the hex ladder (interface band arms) and the polyhedral twin pair
    def _is_prod(r):
        return ((r["pair"].startswith("stationaryDroplet3Drefined:stationaryDroplet3Duniform")
                 and r.get("source") == "interface")
                or r["pair"].startswith("polyDroplet3Drefined_r13p8:polyDroplet3D_r13p8"))
    prod = [r for r in rows if _is_prod(r)]
    if prod:
        cols = [("max_t", "meanMagUPrime", "$\\max_t\\overline{|\\mathbf u|}$"),
                ...
                ("T", "A2hL2Band", "$A_{2h}$")]
        # one row per (pair, N): hex rows by N_fine, the polyhedral pair labelled as such
        keys = sorted({(r["pair"], r["N_CELLS"]) for r in prod}, key=lambda k: (k[0].startswith("poly"), k[1]))
        with open(os.path.join(a.root, a.outdir, "refined_equivalence.tex"), "w") as fh:
            ...
            for pair_key, n in keys:
                rr = [r for r in prod if r["N_CELLS"] == n and r["pair"] == pair_key]
                label = f"{n} (poly, $R/h=12.6$)" if pair_key.startswith("poly") else str(n)
                ...
                fh.write(f"{label} & {steps} & " + " & ".join(cells) +
