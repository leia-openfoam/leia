...
def main(argv):
    ...
    ax.set_title("Face-centered curvature on the CSF force support\n"
                 r"(static circle, exact SDF; faces with $|\mathrm{snGrad}\,\alpha| > 0$)",
                 fontsize=11)
    ax.grid(True, which="both", ls=":", alpha=0.5)
    ax.legend(frameon=False, fontsize=8, loc="center left",
              bbox_to_anchor=(1.01, 0.5))
    fig.tight_layout()
    ppath = os.path.join(figs, "face_curvature_convergence.png")
    ...
    cpath = os.path.join(tables, "face_curvature_orders.csv")
    with open(cpath, "w", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=list(rows[0]))
        w.writeheader(); w.writerows(rows)
    print(f"[facecurv] wrote {cpath} ({len(rows)} rows)")

    tpath = os.path.join(tables, "face_curvature_orders.tex")
