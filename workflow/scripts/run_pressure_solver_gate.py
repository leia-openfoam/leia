#!/usr/bin/env python3
"""Compare production GAMG with strict GAMG/PCG in the frozen pressure gate."""
...
def main() -> int:
    ...
    gate = config["pressure_solver_gate"]
    ...
    if correction_count < pressure_gate.MIN_PERTURBED_NONORTHOGONAL_CORRECTORS:
        raise SystemExit("pressure-solver gate is under-corrected")
    ...
    for resolution in gate["resolutions"]:
        ...
        for variant in gate["variants"]:
            ...
            variant_tokens.update(
                {
                    ...
                    "PRESSURE_SOLVER": variant["pressure_solver"],
                    "PRESSURE_PRECONDITIONER": variant[
                        "pressure_preconditioner"
                    ],
                    "PRESSURE_TOLERANCE": f"{float(variant['pressure_tolerance']):.12g}",
                    "PRESSURE_REL_TOL": str(variant["pressure_rel_tol"]),
                }
            )
            ...
            rows.append(
                {
                    ...
                    "pressure_tolerance": variant["pressure_tolerance"],
                    "pressure_rel_tol": variant["pressure_rel_tol"],
            ...
            print(
                f"[pressure-solver] N={resolution} {variant['id']}: "
                f"status={solve_status}, max|U|={max_u:.12g} m/s"
            )

    result = study_dir / "pressure_solver_gate.csv"
    publication = (REPO / gate["publication_table"]).resolve()
    write_rows(result, rows)
    write_rows(publication, rows)
    print(f"[pressure-solver] result: {result}")
    print(f"[pressure-solver] publication: {publication}")
    return 0
