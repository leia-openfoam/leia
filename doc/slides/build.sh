...
#        ▼
#   doc/slides/index.html            (vertical section stacks)
#   doc/slides/index-linear.html     (flat, front-to-back reading)
#
# Both outputs are ONE self-contained file each: local figures/*.png are inlined
# as base64 and the CDN assets (reveal.js, theme, MathJax) are fetched and inlined
# so the deck opens offline. Requires only python3 (standard library) + a network
# connection ON THE FIRST BUILD to inline the CDN assets; without network the deck
# still builds but keeps CDN <link>/<script> tags (so it then needs the internet
# to render). Regenerating the FIGURES is a separate, heavier step -- see README.md.
#
# Run from anywhere:  bash doc/slides/build.sh
set -euo pipefail
